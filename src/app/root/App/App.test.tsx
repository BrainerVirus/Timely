...
vi.mock("@/app/desktop/TauriService/tauri", async () => {
  ...
  return {
    ...
    loadAppPreferences: vi.fn(async () => ({
      ...
      lastInstalledVersion: "0.1.0-beta.12",
      lastSeenReleaseHighlightsVersion: "0.1.0-beta.12",
...
describe("App", () => {
  it("renders the dashboard shell with NavRail and TopBar", async () => {
    ...
    expect(screen.getByText("Beta 0.1.0-beta.12")).toBeInTheDocument();
  });
  ...
  it("opens the about dialog when the desktop event is emitted", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("v0.1.0-beta.12")).toBeInTheDocument();
    });
  });
  ...
  it("waits for onboarding to finish before showing release highlights for an upgrade", async () => {
    ...
    expect(await screen.findByText("Beta.12 brings YouTrack and smarter sync")).toBeInTheDocument();
  });
